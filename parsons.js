(function($, _) { // wrap in anonymous function to not show some helper variables
   ...
   ParsonsWidget.prototype.createHTMLFromHashes = function(solutionHash, trashHash) {
    ...
    ParsonsWidget.prototype.codeLineToHTMLUsingColourText = function(codeline) {
        ...
        if (this.options.colored_keyword){
          if(codeline.code.substring(0,4) === "FROM"){
            ...
            return '<li id="' + codeline.id + '" style="color:MediumSeaGreen;">' + codeline.code + '<\/li>';
          } else if(codeline.code.substring(0,6) === "SELECT"){
            console.log(codeline.code.substring(0,6))
            return '<li id="' + codeline.id + '" style="color:Tomato;">' + codeline.code + '<\/li>';
          } else if(codeline.code.substring(0,5) === "WHERE"){
            console.log(codeline.code.substring(0,5))
            return '<li id="' + codeline.id + '" style="color:DodgerBlue;">' + codeline.code + '<\/li>';
          }
        }
        return '<li id="' + codeline.id + '" style="color:Black;"" >' + codeline.code + '<\/li>';
    };
    ...
    ParsonsWidget.prototype.codeLineToHTMLPanel = function(codeline) {
      ...
      if (this.options.colored_keyword){
        if(codeline.code.substring(0,4) === "FROM"){
          console.log(codeline.code.substring(0,4))
          return ['<li id="' + codeline.id + '" style="color:MediumSeaGreen;">' + codeline.code + '<\/li>', 1];
        } else if(codeline.code.substring(0,6) === "SELECT"){
          console.log(codeline.code.substring(0,6))
          return ['<li id="' + codeline.id + '" style="color:Tomato;">' + codeline.code + '<\/li>',2];
        } else if(codeline.code.substring(0,5) === "WHERE"){
          console.log(codeline.code.substring(0,5))
  ...
  ParsonsWidget.prototype.getLineBasedOnType = function(codelineIDs, destinationID,typeNum) {
    var lineHTML = [];
      for(var id in codelineIDs) {
          var line = this.getLineById(codelineIDs[id]);
          var results = this.codeLineToHTMLPanel(line);
          if (results[1] == typeNum) {
            lineHTML.push(results[0]);
          }
      }
      return '<ul id="ul-' + destinationID + '" class="ui-sortable" style>'+lineHTML.join('')+'</ul>';
  }
   ...
   ParsonsWidget.prototype.createHTMLFromLists = function(solutionIDs, trashIDs) {
    ...
    if (window.prettyPrint && (typeof(this.options.prettyPrint) === "undefined" || this.options.prettyPrint)) {
     ...
     if (this.options.trashId) {
       var trash1 = $("#ul-" + this.options.trashId).sortable(
         {
    ...
    if (this.options.trashId) {
      var trash2 = $("#ul-" + this.options.trashId+"1").sortable(
        {
          connectWith: sortable,
          start: function() { that.clearFeedback(); },
          receive: function(event, ui) {
    ...
    if (this.options.trashId) {
      var trash3 = $("#ul-" + this.options.trashId+"3").sortable(
        {
          connectWith: sortable,
          start: function() { that.clearFeedback(); },
          receive: function(event, ui) {
    ...
    if (this.options.trashId) {
      var trash4 = $("#ul-" + this.options.trashId+"4").sortable(
        {
          connectWith: sortable,
          start: function() { that.clearFeedback(); },
          receive: function(event, ui) {
